...
import { Save, ChevronDown, ChevronRight, Users } from 'lucide-react';
...
function LinhaInput({ label, value, onChange, receitaBruta, placeholder }: {
  ...
  return (
    <div style={{
      ...
      padding: '6px 16px', borderBottom: '1px solid #f8fafc', gap: 8,
    }}>
...
function LinhaSubtotal({ label, value, receitaBruta, destaque = false }: {
  ...
  return (
    <div style={{
      ...
      background: destaque ? '#f0f9ff' : '#f8fafc',
      ...
      <span style={{
        width: 140, textAlign: 'right', fontSize: 14, fontWeight: 700,
        fontFamily: 'monospace', color: value >= 0 ? '#059669' : '#dc2626',
      }}>
        {formatBRL(value)}
      </span>
  ...
  return (
    <button
      ...
      style={{
        ...
        padding: '10px 16px', background: cor, border: 'none', cursor: 'pointer',
        textAlign: 'left', borderBottom: aberta ? '1px solid rgba(0,0,0,0.06)' : 'none',
      }}
    >
      {aberta ? <ChevronDown size={14} color="#fff" /> : <ChevronRight size={14} color="#fff" />}
      <span style={{ flex: 1, fontSize: 13, fontWeight: 700, color: '#fff', textTransform: 'uppercase', letterSpacing: 0.5 }}>
        {titulo}
      </span>
      {total !== undefined && (
        <span style={{ fontSize: 13, fontFamily: 'monospace', color: 'rgba(255,255,255,0.9)', fontWeight: 600 }}>
          {formatBRL(total)}
        </span>
      )}
    </button>
...
export default function FormDRE({ periodo, recursos, onSalvar }: Props) {
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <h3 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: '#1e293b' }}>
          DRE — {MESES[periodo.mes - 1]} {periodo.ano}
        </h3>
        <button
          onClick={() => onSalvar(linhas, obs, alocacoes)}
          style={{
      ...
      <div style={{
        ...
        background: '#1e293b', borderRadius: '8px 8px 0 0', gap: 8,
      }}>
      ...
      <div style={{ border: '1px solid #e2e8f0', borderTop: 'none', borderRadius: '0 0 12px 12px', overflow: 'hidden', background: '#fff' }}>
          ...
          <div style={{ padding: '10px 16px', borderBottom: '1px solid #fef3c7', background: '#fffbeb' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <span style={{ fontSize: 12, color: '#92400e', fontWeight: 600 }}>
                {alocacoes.length > 0
                  ? `${alocacoes.length} recurso(s) alocado(s) neste período`
                  : 'Alocar horas dos recursos para calcular custos automaticamente'}
              </span>
              <button
              ...
              <div style={{ marginTop: 8 }}>
                {alocacoes.map(a => (
                  <div key={a.recursoId} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '3px 0', borderBottom: '1px solid #fde68a' }}>
                    <span style={{ padding: '1px 5px', borderRadius: 3, fontSize: 10, fontWeight: 700, background: COR_TIPO[a.tipoSnapshot] + '18', color: COR_TIPO[a.tipoSnapshot] }}>
                      {a.tipoSnapshot}
                    </span>
        ...
        {abertas.financeiro && <>
          <LinhaInput label="Despesas Financeiras (juros, IOF, tarifas)" value={linhas.despesasFinanceiras} onChange={v => set('despesasFinanceiras', v)} receitaBruta={rol} />
        ...
        <div style={{
          ...
          background: dre.lucroLiquido >= 0 ? '#f0fdf4' : '#fef2f2',
          borderTop: `3px solid ${dre.lucroLiquido >= 0 ? '#16a34a' : '#dc2626'}`,
        }}>
          <span style={{ flex: 1, fontSize: 16, fontWeight: 800, color: '#1e293b' }}>
            = LUCRO LÍQUIDO DO EXERCÍCIO
          </span>
          <span style={{
            width: 140, textAlign: 'right', fontSize: 16, fontWeight: 800,
            fontFamily: 'monospace', color: dre.lucroLiquido >= 0 ? '#16a34a' : '#dc2626',
          }}>
